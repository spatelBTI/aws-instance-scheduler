...
class ServerStates:
    ...
    def load(self, account, region):
        ...
        try:
            resp = self.state_table.get_item_with_retries(Key={
                ServerStates.INSTANCE_TABLE_NAME: self._service,
                ServerStates.INSTANCE_TABLE_ACCOUNT_REGION: self._current_account_region
            }, ConsistentRead=True)
            item = resp.get("Item", {})
        ...
        # time of last update
        if ServerStates.INSTANCE_TABLE_TIMESTAMP in item:
            self._timestamp = item[ServerStates.INSTANCE_TABLE_TIMESTAMP]
        else:
            self._timestamp = Decimal(time.time())

        # instance states, one column per instance
        self._state_info = {i: item[i] for i in item if
                            i not in [ServerStates.INSTANCE_TABLE_TIMESTAMP, ServerStates.INSTANCE_TABLE_NAME,
                                      ServerStates.INSTANCE_TABLE_ACCOUNT_REGION, ServerStates.INSTANCE_TABLE_PURGE]}

        # items to purge
        if ServerStates.INSTANCE_TABLE_PURGE in item:
            self._server_to_purge = item[ServerStates.INSTANCE_TABLE_PURGE]

    def set_instance_state(self, server_id, new_state):
        """
        Sets the state of an instance
        :param server_id: id of the instance
        :param new_state: new state
        :return:
        """
        # get stored state
        state = self._state_info.get(server_id, None)
        # only update if changed
        if not state or state != new_state:
            self._state_info[server_id] = new_state
            self._dirty = True
    ...
    def delete_server_state(self, server_id):
        ...
        :return:
        """
        if server_id in self._state_info:
            del self._state_info[server_id]
            if server_id in self._server_to_purge:
                self._server_to_purge.remove(server_id)
            self._dirty = True
    ...
    def save(self):
        ...
        if self._dirty:
            ...
            data = {
                ServerStates.INSTANCE_TABLE_NAME: self._service,
                ServerStates.INSTANCE_TABLE_ACCOUNT_REGION: self._current_account_region,
                ServerStates.INSTANCE_TABLE_TIMESTAMP: Decimal(time.time())
            }
            ...
            # instances to purge
            if len(self._server_to_purge) > 0:
                data[ServerStates.INSTANCE_TABLE_PURGE] = self._server_to_purge
    ...
    def cleanup(self, instances):
        ...
        if (Decimal(time.time()) - Decimal(self._timestamp)) > ServerStates.cleanup_interval:
            ...
            for i in stored_instances:
                ...
                if i not in instances:
                    ...
                    if i in self._server_to_purge:
                        # remove instance
                        del self._state_info[i]
                        self._server_to_purge.remove(i)
                        ...
                        self._server_to_purge.add(i)
                        self._dirty = True
                        self._logger.info(INF_MOVING_FOR_PURGE, i)
                elif i in self._server_to_purge:
